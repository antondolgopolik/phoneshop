...
public class AjaxCartController {
    ...
    @ResponseBody
    public Map<String, Object> addCartItem(@PathVariable Long phoneId,
    ...
    @ResponseBody
    public Map<String, Object> updateCartItems(@RequestBody Map<Long, Integer> updates) {
        // Check updates
    ...
    @ResponseBody
    public Map<String, Object> deleteCartItem(@PathVariable Long phoneId) {
        // Check parameters
        checkPhoneId(phoneId);
        // Remove from cart
        cartService.delete(phoneId);
        // Response
        return responseWithCartInfo();
    }
    ...
    private void checkUpdates(Map<Long, Integer> updates) {
        ...
        updates.forEach((phoneId, quantity) -> {
            if (quantity < 0) {
                errors.put(phoneId, "Quantity cannot be negative!");
            }
        });
        if (!errors.isEmpty()) {
